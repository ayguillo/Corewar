...
/*   Updated: 2019/07/18 17:46:54 by bopopovi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_lfork(t_vm *vm, t_proc *process, t_param *params, t_op op)
{
	...
	(void)op;
	local_dbg(l_dbg, "{magenta}EXECUTING OP_LFORK{eoc}\n");
	fork_addr = (process->pc + (params[0].val)) % MEM_SIZE;
	local_dbg(l_dbg, "Fork address : 0x%08x (%u)\n", fork_addr, fork_addr);
	local_dbg(l_dbg, "(PC [%u] + ((P1 [%u])) %% MEM_SIZE)",
		process->pc, params[0].val);
	add_process(vm, process->player, fork_addr, process);
	local_dbg(l_dbg, "{magenta}OP_LFORK END{eoc}\n\n");
}
