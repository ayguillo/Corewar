...
/*   Updated: 2019/07/10 16:31:38 by ayguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_paramd(char **split, t_op *op, int size, t_gnl *gnl)
{
	...
	param = ft_filllabel(op, split, 1, gnl);
...
int			ft_paramld(char **split, t_op *op, t_gnl *gnl)
{
	...
	{
		if (split[2][0] != 'r')
			return (ft_errorparams(gnl, 1, split[2], 1));
...
int			ft_param3rg(char **split, t_op *op, t_gnl *gnl)
{
	...
	{
		if (split[i][0] != 'r')
			return (ft_errorparams(gnl, 1, split[i], i - 1));
