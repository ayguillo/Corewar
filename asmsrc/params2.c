...
/*   Updated: 2019/07/02 17:01:47 by ayguillo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_paramldi(char **split, t_op *op)
{
	...
	if (!(ft_rgidd(split, op, DIR_SIZE / 2, 1)))
		return ;
	if (!(ft_idd(split, op, DIR_SIZE / 2, 2)))
		return ;
...
void	ft_paramsti(char **split, t_op *op)
{
	...
	if (!(ft_rgidd(split, op, DIR_SIZE / 2, 2)))
		return ;
	if (!(ft_idd(split, op, DIR_SIZE / 2, 3)))
		return ;
	if (!split[1] || !split[2] || !split[3])
		ft_printf("Bad argument\n");
}
...
void	ft_paramcomp(char **split, t_op *op)
{
	if (split[0])
		ft_fillinstop(split[0], op);
	if (!(ft_rgidd(split, op, DIR_SIZE, 1)))
		return ;
	if (!(ft_rgidd(split, op, DIR_SIZE, 2)))
		return ;
